...
import { getOmisellWebhookLogs } from "@/services/omisell/omisell";
import type { OmisellWebhookLog } from "@/services/omisell/types";
...
export default function OmisellWebhookLogs() {
  const [logs, setLogs] = useState<OmisellWebhookLog[]>([]);
  ...
  const loadLogs = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await getOmisellWebhookLogs({ limit: PAGE_SIZE, offset });
      setLogs(data.logs);
      setTotal(data.total);
    } catch (loadError) {
      ...
      <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-0.5 text-xs text-gray-100">
            รายการที่ Omisell ส่งเข้ามาล่าสุด
          </p>
        </div>
...
function CustomerCell({ log }: { log: OmisellWebhookLog }) {
  if (log.member) {
    return (
      <>
...
function LogCard({ log }: { log: OmisellWebhookLog }) {
  return (
...
function LogStatusBadge({ log }: { log: OmisellWebhookLog }) {
